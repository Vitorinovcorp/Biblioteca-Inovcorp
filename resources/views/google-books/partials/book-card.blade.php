...
<div class="bg-white rounded-lg shadow-md hover:shadow-lg transition-all duration-300 flex flex-col h-full border border-gray-100">
    ...
    <div class="flex justify-center items-center p-4 bg-gray-50 rounded-t-lg h-48">
        @if($thumbnail)
            <img src="{{ $thumbnail }}" class="h-full object-contain" alt="{{ $title }}">
        @else
    ...
    <div class="p-4 flex flex-col flex-1">
        ...
        <p class="text-gray-600 text-sm mb-2 line-clamp-2">
            <i class="fas fa-user mr-1 text-gray-400"></i> {{ Str::limit($authors, 60) }}
        </p>
        
        <div class="text-gray-500 text-xs mb-3 space-y-1">
            @if($publishedDate && $publishedDate != 'Data não informada')
                <div><i class="far fa-calendar-alt mr-1"></i> {{ $publishedDate }}</div>
            @endif
            @if($isbn)
                <div><i class="fas fa-barcode mr-1"></i> ISBN: {{ $isbn }}</div>
            @endif
        </div>
        ...
        <div class="mt-auto pt-2">
            <button type="button" 
                ...
                data-authors="{{ htmlspecialchars($authors, ENT_QUOTES, 'UTF-8') }}"
                data-isbn="{{ $isbn }}"
