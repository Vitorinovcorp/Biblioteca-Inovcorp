    ...
    <div class="bg-gradient-to-r from-purple-600 to-indigo-600 rounded-lg shadow-lg p-8 mb-8">
        <div class="text-center text-white">
            ...
            <p class="text-lg opacity-90">
                Baseado no livro: <strong class="font-semibold">{{ $livro->nome }}</strong>
            </p>
            <p class="mt-2">
        ...
        <div class="flex items-center space-x-4">
            <div class="w-24 h-32 bg-gray-100 rounded-lg flex items-center justify-center">
                @if($livro->imagem_capa)
                    ...
                    @endphp
                    <img src="{{ $imageUrl }}" alt="{{ $livro->nome }}" class="w-full h-full object-cover rounded-lg">
                @else
                    <i class="fas fa-book fa-3x text-gray-400"></i>
                @endif
            </div>
            <div>
                <h2 class="text-2xl font-bold text-gray-800">{{ $livro->nome }}</h2>
                <p class="text-gray-600 mt-1">
                    <i class="fas fa-user mr-1"></i>
                    {{ $livro->autores->pluck('nome')->implode(', ') ?: 'Autor não informado' }}
                </p>
                @if($livro->bibliografia)
                    <p class="text-gray-500 text-sm mt-2">{{ Str::limit($livro->bibliografia, 200) }}</p>
                @endif
            </div>
        </div>
    </div>
    ...
    <div class="mb-8">
        <div class="flex justify-between items-center mb-6">
            ...
            <span class="text-gray-500">
                {{ $recommendations->count() }} recomendações encontradas
            </span>
        </div>
            ...
            <div class="bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition-shadow duration-300 group flex flex-col h-full">
                <a href="{{ route('livros.show', $recommendation->id) }}" class="block flex-1">
                    ...
                    <div class="p-4">
                        ...
                        @if($recommendation->bibliografia)
                        <p class="text-xs text-gray-500 line-clamp-2 mb-3">
                            {{ Str::limit($recommendation->bibliografia, 100) }}
                        </p>
                        @endif
                        ...
                        @if(isset($similarityScores[$recommendation->id]))
                        <div class="mt-2 pt-2 border-t border-gray-100">
                            <div class="flex items-center justify-between text-xs">
                                ...
                                <span class="font-semibold text-purple-600">
                                    {{ round($similarityScores[$recommendation->id] * 100) }}%
                                </span>
                            </div>
                        </div>
                        @endif
        ...
        <div class="bg-gray-50 rounded-lg p-12 text-center">
            ...
            <a href="{{ route('livros.index') }}" class="inline-block mt-4 text-purple-600 hover:text-purple-700">
